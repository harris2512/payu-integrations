...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                CamearActivity.GALLERY_DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e(CamearActivity.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        + CamearActivity.GALLERY_DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        File mediaFile;
        if (type == CamearActivity.MEDIA_TYPE_IMAGE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_" + timeStamp + "." + CamearActivity.IMAGE_EXTENSION);
        } else if (type == CamearActivity.MEDIA_TYPE_VIDEO) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "VID_" + timeStamp + "." + CamearActivity.VIDEO_EXTENSION);
        } else {
            return null;
        }
